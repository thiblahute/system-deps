...
#[test]
fn conditional_inherit() -> Result<(), Error> {
    // Child has cfg() that resolves, parent inherits the resolved value
    let pkgs = vec![
        Package {
            name: "main",
            deps: vec!["dep"],
    ...
    let test = Test::new("conditional_inherit", pkgs)?;
    assert_eq!(test.check("dep")?, &toml::toml![value = "child-cfg"]);

    // Parent overrides the cfg()-resolved value from the child
    let pkgs = vec![
        Package {
            name: "main",
            deps: vec!["dep"],
    ...
    let test = Test::new("conditional_inherit_override", pkgs)?;
    assert_eq!(test.check("dep")?, &toml::toml![value = "parent-override"]);

    Ok(())
}

#[test]
fn conditional_nested() -> Result<(), Error> {
    // cfg() inside a dependency table (nested) — matching
    let pkgs = vec![Package {
        name: "dep",
        deps: vec![],
        config: toml::from_str(
            ...
            [package.metadata.system-deps.dep.'cfg(all())']
            url = "platform-url"
        "#,
        )?,
    }];
    let test = Test::new("conditional_nested_true", pkgs)?;
    assert_eq!(
        test.check("dep")?,
        &toml::toml![
            url = "platform-url"
    ...
    // cfg() inside a dependency table (nested) — not matching
    let pkgs = vec![Package {
        name: "dep",
        deps: vec![],
        config: toml::from_str(
            ...
            [package.metadata.system-deps.dep.'cfg(any())']
            url = "platform-url"
        "#,
        )?,
    }];
    let test = Test::new("conditional_nested_false", pkgs)?;
    assert_eq!(
        test.check("dep")?,
        &toml::toml![
            url = "default"
